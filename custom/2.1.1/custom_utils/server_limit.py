def named_server_limit_per_user_exceeded(user, options):
    ...
    service_limit = vo_config.get("Services").get(service_type).get(service_option).get("max_per_user")
    if service_limit:
        current = 0
        for spawner in named_spawners:
            if spawner.user_options.get("service", "") == service:
                current += 1
    ...
    if system_limit:
        current = 0
        for spawner in named_spawners:
            if spawner.user_options.get("system", "") == system:
                current += 1
