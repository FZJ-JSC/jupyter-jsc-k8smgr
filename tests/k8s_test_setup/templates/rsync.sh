...
while true; do
    ...
    if [[ -f <DIR>/<ID>/pids/port-forward_unicoremgr.pid ]]; then
        rsync -r -c -q -b -e "ssh -p 2224 -i <DIR>/<ID>/keypairs/devel -oLogLevel=ERROR -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no" unicoremgr@localhost:/home/unicoremgr/web <DIR>/<ID>/rsync/unicoremgr || true
    fi
